...
static fz_font *load_noto(fz_context *ctx, const char *a, const char *b, const char *c, int idx)
{
	...
	fz_try(ctx)
	{
        ...
        if (fz_file_exists(ctx, buf))
            fz_snprintf(buf, sizeof buf, "/storage/emulated/0/Fonts/%s%s%s.ttf", a, b, c);
        if (fz_file_exists(ctx, buf))
            fz_snprintf(buf, sizeof buf, "/storage/emulated/0/Fonts/%s%s%s.ttc", a, b, c);
        if (fz_file_exists(ctx, buf))
            fz_snprintf(buf, sizeof buf, "/storage/emulated/0/Fonts/%s.ttf", a);
        if (fz_file_exists(ctx, buf))
            fz_snprintf(buf, sizeof buf, "/storage/emulated/0/Fonts/%s.ttc", a);
        if (!fz_file_exists(ctx, buf))
            fz_snprintf(buf, sizeof buf, "/storage/sdcard/Fonts/%s%s%s.ttf", a, b, c);
        if (!fz_file_exists(ctx, buf))
            fz_snprintf(buf, sizeof buf, "/storage/sdcard/Fonts/%s%s%s.ttc", a, b, c);
		if (fz_file_exists(ctx, buf))
