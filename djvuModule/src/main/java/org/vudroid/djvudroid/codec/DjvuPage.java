...
public class DjvuPage implements CodecPage {
    ...
    public Bitmap renderBitmap(Rect cropBound, int width, int height, RectF pageSliceBounds, float scale) {
        ...
        Log.d("TAG", String.format("page:%s, scale:%s-%s, patchX:%s, patchY:%s, width:%s, height:%s, %s, %s",
                pageNumber, scale, scale * getWidth(), patchX, patchY, width, height, cropBound, pageSliceBounds));
        ...
        int pageW = (int) (getWidth() * scale);
        int pageH = (int) (getHeight() * scale);
        renderPageDirect(pageHandle, contextHandle, pageW, pageH, (int) patchX, (int) patchY, width, height, buffer);
